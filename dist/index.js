...
const pastNumsOutput = document.querySelector('.past-nums ul');
...
const changeBgColor = (nums) => {
    let str = '';
    nums.forEach(num => {
        str = str + `${num}`;
    });
    const color1 = `#` + str.substring(0, 6);
    const color2 = `#` + str.substring(str.length, str.length - 6);
    body.style.background = `linear-gradient(45deg, ${color1}, ${color2})`;
};
const addToPastNumsArr = (nums) => {
    let str = '';
    nums.forEach(num => {
        str = str + `${num},`;
    });
...
const generateNum = () => Math.floor(Math.random() * 70 + 1);
const updateDom = (nums) => {
